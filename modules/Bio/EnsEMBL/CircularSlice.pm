...
=head2 get_all_Operons
  ...
  Arg [5]    : (optional) string $biotype
               The biotype of the genes to retrieve.
  Example    : @genes = @{$slice->get_all_Operons};
  Description: Retrieves all genes that overlap this slice.
  Returntype : listref of Bio::EnsEMBL::Operons
  Exceptions : none
  Caller     : none
  Status     : Stable
...
sub get_all_Operons{
  ...
  my $opa;
   if($dbtype) {
     my $db = $reg->get_db($self->adaptor()->db(), $dbtype);
     if(defined($db)){
       $opa = $reg->get_adaptor( $db->species(), $db->group(), "Operon" );
     }
     else{
       $opa = $reg->get_adaptor( $self->adaptor()->db()->species(), $dbtype, "Operon" );
     }
     if(!defined $opa) {
       warning( "$dbtype operons not available" );
       return [];
     }
 } else {
    $opa =  $self->adaptor->db->get_OperonAdaptor();
 }
## circular BOF
  ...
  my (@arr, @arr1, @arr2, @arr2f);
  @arr1 = @{
    $opa->fetch_all_by_Slice( $sl1, $logic_name, $load_transcripts,
                             $source, $biotype ) };
  @arr2 = @{
    $opa->fetch_all_by_Slice( $sl2, $logic_name, $load_transcripts,
                             $source, $biotype ) };
  #Due to this piece of code the second half of each overlapping the ori feature will be missing in Alignments (image) page where the two parts of the slice are displayed separately  
  foreach my $g (@arr2) {
    if ($g->seq_region_start() < $g->seq_region_end()){push(@arr2f, $g)}
  }
