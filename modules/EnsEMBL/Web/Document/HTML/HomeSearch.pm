...
## EG  
  ...
  my $species_name        = $page_species eq 'Multi' ? 'all species' : $species_defs->species_display_label($hub->species);
## /EG
...
## /EG
  ...
  $q_params->{'value'} = "Search $species_name..."; ## EG unless $is_home_page;
  ...
  my $html; 
  if ($is_home_page) {
    $html = '<div id="SpeciesSearch" class="js_panel home-search-flex"><h3 class="first">Search for a gene</h3>';
  } else {
    $html = '<div id="SpeciesSearch" class="js_panel">';
  }
  
  return $html . sprintf '<input type="hidden" class="panel_type" value="SearchBox" />%s</div>', $form->render;
}
