...
## EG - hack to force Bacteria to allow attachments without valid assembly lookup
##      FIXME: need to make assembly_lookup work with multi-species dbs
          #my ($data_species, $assembly) = @{$ensembl_assemblies{$_}||[]};         
          my ($data_species, $assembly) = ($hub->data_species, '');
          if (1 or $assembly) { # always do it
