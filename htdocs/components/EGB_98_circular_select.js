        ...
        if ($('img.circularImage', this.el).length) {
            ...
            $('img.circularImage', this.el).each(function (intIndex) {
              ...
              $(this).load(  function () {
                ...
                var image = paper.image($(this).attr('src'), margin, margin, $(this).prop('width'), $(this).prop('height'));

                var fudge = $.browser.msie ? 1 : 0; // make sure centred in ie
                //console.log('ID '  + $(this).attr('id') );
                
                if (bp[2] <= 100000) {
                  // whole region can be displayed - don't use piechart selector
                  var species = window.location.href.replace(/^(?:\/\/|[^\/]+)*\//, "").split('/')[0];
                  var url = "/" + species + '/Location/View?r=' + reg_type[1] + ':1-' + bp[2];
                  image.click(function(){document.location.href = url}).attr({cursor: "pointer"});;
                } else {
                  // attach piechart selector
                  if (ifcirc === 0) {
